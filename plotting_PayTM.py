...
def plot_crash_detections(
    ...
    is_date_in_interval = (time_index_derivs > pd.to_datetime(start_date).tz_localize(None)) & (time_index_derivs < pd.to_datetime(end_date).tz_localize(None))
...
def plot_crash_comparisons(
    ...
    # define time intervals to plots
    is_date_in_interval = (time_index_derivs > pd.Timestamp(start_date).tz_localize(None)) & (
        time_index_derivs < pd.Timestamp(end_date).tz_localize(None)
    )
    print(type(start_date))
